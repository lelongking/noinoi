...
#  created: -> FlowRouter.go('/merchant') unless Meteor.userId() is null or (Session.get('autoNatigateDashboardOff'))
  created: ->
...
#        Meteor.loginWithPassword $account.val(), $secret.val(), (error) -> FlowRouter.go('/merchantWizard') if !error
