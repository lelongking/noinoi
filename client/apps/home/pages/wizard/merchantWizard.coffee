...
#    FlowRouter.go('/') if Meteor.userId() is null
#    unless Session.get('merchantPackages')?.user is Meteor.userId() then FlowRouter.go('/dashboard')
#    if Session.get('merchantPackages')?.merchantRegistered then FlowRouter.go('/dashboard')
#
...
#      FlowRouter.go('/dashboard') if Session.get("currentProfile").merchantRegistered
#
