    ...
    dataLists: ->
      ...
      if importLists.length > 0
        for key, value of _.groupBy(importLists, (item) -> moment(item.sortDate).format('MM/YYYY'))
          ...
          dataLists.groups.push({description: 'NHẬP KHO THÁNG ' + key, details: value, totalCash: totalCash})
      else
        dataLists.groups.push({description: 'NHẬP KHO THÁNG ' + moment().format('MM/YYYY'), details: [], totalCash: 0})
