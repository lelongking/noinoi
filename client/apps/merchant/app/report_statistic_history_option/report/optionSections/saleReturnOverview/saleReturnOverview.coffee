    ...
    dataLists: ->
      ...
      if saleReturnLists.length > 0
        for key, value of _.groupBy(saleReturnLists, (item) -> moment(item.sortDate).format('MM/YYYY'))
          ...
          dataLists.groups.push({description: 'TRẢ HÀNG THÁNG ' + key, details: value, totalCash: totalCash})
      else
        dataLists.groups.push({description: 'TRẢ HÀNG THÁNG ' + moment().format('MM/YYYY'), details: [], totalCash: 0})
      dataLists
