...
Wings.defineApp 'importReturnOverview',
  created: ->
    self = this
    self.autorun ()->
  ...
  helpers:
    dataLists: ->
      ...
      importReturnLists = Schema.returns.find({
      ...
      if importReturnLists.length > 0
        for key, value of _.groupBy(importReturnLists, (item) -> moment(item.sortDate).format('MM/YYYY'))
          totalCash = 0
          for item, index in value
            item.count = index+1
            totalCash += item.finalPrice
          dataLists.groups.push({description: 'TRẢ NHẬP KHO THÁNG ' + key, details: value, totalCash: totalCash})
      else
        dataLists.groups.push({description: 'TRẢ NHẬP KHO THÁNG ' + moment().format('MM/YYYY'), details: [], totalCash: 0})
      dataLists
