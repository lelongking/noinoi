    ...
    dataLists: ->
      ...
      if orderLists.length > 0
        for key, value of _.groupBy(orderLists, (item) -> moment(item.sortDate).format('MM/YYYY'))
          ...
          dataLists.groups.push({description: 'DOANH THU THÁNG ' + key, details: value, totalCash: totalCash})
      else
        dataLists.groups.push({description: 'DOANH THU THÁNG ' + moment().format('MM/YYYY'), details: [], totalCash: 0})
      dataLists
