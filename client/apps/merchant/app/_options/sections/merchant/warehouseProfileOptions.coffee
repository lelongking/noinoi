...
Wings.defineHyper 'warehouseInfoOptions',
  ...
  helpers:
    warehouseProfile: -> Session.get('warehouse')
...
#-----------------------------------------------------------------------------------------------------------------------
checkUpdateWarehouseProfileEdit = (event, template) ->
  if warehouseData = Session.get('warehouse')
...
syncWarehouseProfileData = (event, template)->
  if Template.instance().showSyncCommand.get()
    warehouseData        = Session.get('warehouse')
    ...
    option = $set:{}
    option.$set['warehouses.0.name']        = warehouseName if warehouseName isnt (warehouseData.name ? '')
    option.$set['warehouses.0.address']     = warehouseAddress if warehouseAddress isnt (warehouseData.address ? '')
    option.$set['warehouses.0.description'] = warehouseDescription if warehouseDescription isnt (warehouseData.description ? '')
    option.$set['warehouses.0.phone']       = warehousePhone if warehousePhone isnt (warehouseData.phone ? '')
